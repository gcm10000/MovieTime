    ...
    class Commands
    {
        ...
        public int InsertWatch(Watch watch)
        {
            try
            {
                ...
                sqlCommand.CommandText = "INSERT INTO [dbo].[Watch] ([Title],[TitleOriginal],[Duration],[Synopsis],[IDTheMovieDB],[IDIMdb],[Genre],[PosterPicture],[BackdropPicture],[Date],[IsMovie]) " +
                    "VALUES(@Title,@TitleOriginal,@Duration,@Synopsis,@IDTheMovieDB,@IDIMdb,@Genre,@PosterPicture,@BackdropPicture,@Date,@IsMovie)";
                ...
                CommandSelect.CommandText = "SELECT TOP 1 [idWatch] FROM [dbo].[Watch] ORDER BY ID DESC";
                int idWatch = 0;
        ...
        public int InsertSubtitle(Subtitle Subtitle)
        {
            try
            {
                ...
                System.Data.SqlClient.SqlCommand CommandSelect = new System.Data.SqlClient.SqlCommand();
                CommandSelect.Connection = conn;
                CommandSelect.CommandText = "SELECT TOP 1 [idSubtitle] FROM [dbo].[Subtitle] ORDER BY ID DESC";
                int id = 0;
                using (System.Data.SqlClient.SqlDataReader reader = CommandSelect.ExecuteReader())
                {
                    if (reader.Read())
